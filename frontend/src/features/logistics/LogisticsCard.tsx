...
import {
  CheckCircle2, Clock, PackageOpen, AlertOctagon, RefreshCw, Truck,
} from 'lucide-react'
...
function KpiTile({ label, value, icon, colorClass, sub }: KpiProps) {
  return (
    <div className="flex items-center gap-3 p-3.5 rounded-xl bg-slate-50 dark:bg-slate-700/40 border border-slate-100 dark:border-slate-600/40">
      <div className={`h-9 w-9 rounded-lg flex items-center justify-center shrink-0 ${colorClass}`}>
        {icon}
      </div>
      <div className="min-w-0">
        <p className="text-[10px] text-slate-500 dark:text-slate-400 font-medium uppercase tracking-wide leading-tight">
          {label}
        </p>
...
export function LogisticsCard() {
  ...
  const {
    fulfillmentRate, onTimeDelivery,
    overstockItems, stockoutIncidents,
    deliveryPerformance,
  } = data!
  ...
  const kpis: KpiProps[] = [
    {
      ...
      sub:        fulfillmentRate >= 95 ? 'On target' : 'Below target',
    },
    {
      ...
      sub:        onTimeDelivery >= 90 ? 'Healthy' : 'Needs attention',
    },
    {
      label:      'Overstock Items',
      value:      overstockItems,
      icon:       <PackageOpen size={18} className="text-amber-600" />,
      colorClass: 'bg-amber-100 dark:bg-amber-900/40',
      sub:        'Active SKUs',
    },
    {
      label:      'Stockout Incidents',
      value:      stockoutIncidents,
